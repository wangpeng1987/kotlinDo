    ...
    override fun showPage(data: Category, page: Int) {
        ...
        if (title.equals(Constant.strArrays[0])) {
            ...
            for (app:App in appList){
                var android = Android(app._id,app.createdAt,app.desc,app.images,app.publishedAt,app.source,app.type,app.url,app.used,app.who)
            ...
            for (app:休息视频 in appList){
                var android = Android(app._id,app.createdAt,app.desc,mutableListOf(),app.publishedAt,app.source,app.type,app.url,app.used,app.who)
                dataShow.add(android)
            }
